...
logger = logging.getLogger("build_games_shard_standalone")
...
def main(config_path: str) -> Dict[str, Any]:
    ...
    logger.info("=" * 70)
    logger.info("BUILD GAMES SHARD STANDALONE (GamesBuilder diretto, nessun merge finale)")
    logger.info("=" * 70)
    ...
    logger.info("=" * 70)
    logger.info("BUILD GAMES SHARD STANDALONE COMPLETATO")
    logger.info(
    ...
    pending_debug_path = os.path.join(games_output_dir, "games_debug_records.pending.jsonl")
    logger.info(
    ...
    if result.get("enqueued_positions", 0) > 0 and games_cfg.get("save_debug_jsonl", True):
        logger.info(
            f"Debug record grezzi (senza split, in attesa dello split reale) "
            f"accumulati in '{pending_debug_path}'. Il file games_debug.jsonl "
            f"definitivo (con lo split reale train/val/test per ciascuna "
            f"posizione) viene scritto SOLO da DatasetMain.py::_step_finalize_splits, "
            f"dopo registry.build_splits(): questo script standalone non "
            f"calcola piu' uno split via hash locale, per evitare che il "
            f"debug JSONL riporti uno split diverso da quello realmente usato "
            f"in training (vedi GamesBuilder.write_debug_jsonl_from_pending)."
        )
    logger.info(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            ...
            "principale. Non esegue merge/split finale, ne' scrive il debug "
            "JSONL definitivo (quello con lo split reale viene scritto solo "
            "da DatasetMain.py in fase di finalize_splits). skip_games per "
            "ogni fonte viene ricavato da YAML + games_builder_resume.json "
            "(somma), applicato direttamente in questo script prima di "
            "avviare GamesBuilder."
        )
    )
    parser.add_argument(
        "--config",
    ...
    try:
        ...
        logging.getLogger("build_games_shard_standalone").error(f"Errore di configurazione: {e}")
        sys.exit(2)
    except KeyboardInterrupt:
        logging.getLogger("build_games_shard_standalone").warning(
        ...
        logging.getLogger("build_games_shard_standalone").error(f"Interruzione imprevista: {e}")
        sys.exit(1)