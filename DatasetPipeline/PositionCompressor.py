...
PERCHE' QUESTI CAST SONO LOSSLESS (verificato contro PositionGraphSchema.py):
    ...
    - legal_move_mask: [MODIFICATO] gia' torch.bool per costruzione (vedi
      PositionGraphSchema.build_legal_move_mask), NON e' un campo float
      binario come x/edge_attr: va escluso esplicitamente dal branch
      _COMPRESSIBLE_FLOAT_BINARY_FIELDS (che chiama .to(torch.bool) su un
      tensore gia' float 0.0/1.0) per evitare un cast bool->bool superfluo
      ma soprattutto per non dipendere implicitamente dall'ordine dei
      branch se in futuro cambia la lista dei campi float binari. Cade nel
      branch "else: copia invariata": e' gia' nella rappresentazione piu'
      compatta possibile (1 byte/elemento in memoria, torch.save lo
      serializza come bitmap densa), nessuna compressione ulteriore serve.
...
# (edge_index, game_id, ply, y, num_nodes, legal_move_mask, ...) viene
# copiato cosi' com'e', senza compressione: e' gia' un dtype minimale
# (long/int64 scalare, bool denso) o e' un indice PyG non comprimibile
# senza rischio (edge_index).
...
# [NUOVO] Campo bool denso: gia' nella rappresentazione minimale, escluso
# esplicitamente da qualunque branch di compressione lossy/lossless
# ulteriore. Vedi nota "legal_move_mask" sopra.
