...
@dataclass
class GATTimeDecayConfig:
    """Configuration for the time-decay GAT recipe."""
...
def train_gat_time_decay(
    ...
    **overrides,
    ...
    # Per-edge time-diff between consecutive events (NOT node_time_list,
    # see fix note above). This is what TimeAwareGATConv's exponential
    # decay is meant to consume.
    ...
    # node_times=None: no per-node timing feature is needed by
    # DualGATTimeAwareModel.forward (it reads data_event.time expecting
    # the per-edge tensor set via edge_attr below); prepare_data_core_timedif
    # only attaches graph_data.time if node_times is explicitly provided.
    event_feature_list = prepare_data_core_timedif(
        combined_features, core_encode, scaled_time_diffs, node_times=None
    )
    ...
    return {"model": model, "history": history, "attention": best_attention}