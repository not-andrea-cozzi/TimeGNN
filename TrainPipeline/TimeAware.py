...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Training DualGATTimeAwareModel su dataset scacchistico.")
    p.add_argument("--shards-dir", default="Dataset/Train/shards")
    p.add_argument("--checkpoint", default="Dataset/Train/checkpoints/time_aware.pt")
    ...
    # Iperparametri modello (default coerenti con GATTimeDecayConfig)
    ...
    p.add_argument("--lambda-decay", type=float, default=0.01)
...
def main() -> None:
    ...
    logger.info("=" * 60)
    logger.info(f"Training DualGATTimeAwareModel | device={device} | AMP={use_amp}")
    logger.info("=" * 60)
